...
- `/rename` は、native catalog がその session の worker の `status` を報告してから (worker の REPL が起動済みになってから) 送信する。REPL の起動前に届いた入力は Claude の early-input capture に取り込まれ、CR が改行として composer に残るだけで submit されないため (#75)。固定の待ち時間には頼らない。
