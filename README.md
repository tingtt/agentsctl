...
Codex's 5h/weekly usage (shown in the composer's usage line) comes from the same app-server connection, via its `account/rateLimits/read` method. Claude does not currently report usage: no stable, on-demand, headless interface for it was found (see the DesignDoc's Usage section for what was checked), so Claude is simply omitted from the usage line rather than shown as 0%.
