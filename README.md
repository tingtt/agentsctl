...
ChatGPT Project conversations remain visible while the background catalog refresh runs, and stay selectable and openable the whole time -- opening one never waits on that refresh. If a refresh temporarily fails (timeout, an interrupted browser session, and so on), the last successful in-memory catalog remains usable, including Open, until a later complete refresh replaces it; a footer warning surfaces the failure without hiding the still-usable rows. This catalog is kept only in memory for the current `agentsctl` process -- it is not written to disk and starts empty again after a restart.
