...
// involved, unlike AttachClaude. It deliberately does not wait for the
// rename to be confirmed or for the client to exit: the caller
// (Provider.Rename) is expected to start polling `claude agents --json
// --all` for confirmation immediately, and to call the returned cleanup
// func concurrently with that polling rather than after it -- see
// cleanup's own doc comment for why ending the client is a separate,
// unawaited-by-rename-correctness step.
//
...
// (see the doc comment on Provider.Rename in internal/provider/claude).
func SendClaudeRename(ctx context.Context, path, id, name string) (cleanup func(context.Context, time.Duration) error, err error) {
	...
	// same reason AttachClaude always runs an output copy loop
	// concurrently) -- so the client's output is drained for its entire
	// transient lifetime, starting immediately, and thrown away unread.
	// The goroutine exits on its own once child is closed (by cleanup)
	// and every Read on it starts failing.
