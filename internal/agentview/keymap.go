...
// display metadata -- the single source of truth for both *which physical
// Key(s)* a shortcut covers and how it is *displayed* (in the contextual
// footer and the help view -- see footer.go). It carries no behavior: what
// a Key actually means is decided by State.Handle (update.go) given the
// current State, and never by Binding or its callers (see the DesignDoc's
// "Separate terminal protocol decoding from UI semantics" -- a Binding is
// documentation of a physical input plus a membership test, not a dispatch
// table). In particular Binding never stores an Intent or a callback: the
// same Key can mean different things in different States (Esc, Ctrl+X),
// which only State.Handle knows.
//
...
// contextual footer/help text reads their Label instead of re-typing it,
// so a shortcut's physical key can never drift between what's displayed
// and what Handle actually acts on -- there is exactly one definition to
// change.
var (
...
// allBindings is every named shortcut Binding above, used only by
// keymap_test.go's drift guard (every physical Key input_unix.go can
// decode must be either one of these or an explicitly-listed non-shortcut
