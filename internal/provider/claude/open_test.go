...
// to drive Provider.Open end to end without the installed CLI.
...
// TestOpenExplicitDetachReturnsCleanly and TestOpenNaturalExitReturnsCleanly
// exercise Provider.Open end to end against a fake `claude attach` client
// (a tiny shell script, not the installed CLI -- see open_live_test.go for
// that live coverage), the same way
// TestClaudeDetachUsesControlZWhenClientConsumesIt exercises
// detachClaudeClient alone.
func TestOpenExplicitDetachReturnsCleanly(t *testing.T) {
	...
	p := &Provider{Path: script}
	done := make(chan error, 1)
	go func() {
		done <- p.Open(context.Background(), session.Session{Key: session.Key{ID: "id"}}, slave, io.Discard)
	}()
	...
	case <-time.After(5 * time.Second):
		t.Fatal("Open did not return after Ctrl+]")
	}
}

func TestOpenNaturalExitReturnsCleanly(t *testing.T) {
	...
	p := &Provider{Path: script}
	if err := p.Open(context.Background(), session.Session{Key: session.Key{ID: "id"}}, slave, io.Discard); err != nil {
		t.Fatalf("Open err=%v", err)
	}
}

// TestOpenDetachesOnModifyOtherKeysCtrlBracket drives Provider.Open end to
// end (fake client, real PTY) with the outer input sending Ctrl+] in the
// exact byte sequence captured from a real, failing iTerm2 session (issue
// #4) -- xterm's modifyOtherKeys form -- instead of the classic literal
// byte, proving the fix at the same level TestOpenExplicitDetachReturnsCleanly
// already covers for the classic byte.
func TestOpenDetachesOnModifyOtherKeysCtrlBracket(t *testing.T) {
	script := writeFakeClaudeAttachScript(t, "stty raw -echo; dd bs=1 count=1 of=/dev/null 2>/dev/null; exit 0")
	master, slave, err := creackpty.Open()
	if err != nil {
		t.Fatal(err)
	...
	case <-time.After(5 * time.Second):
		t.Fatal("Open did not return after a modifyOtherKeys encoded Ctrl+]")
	}
}
