...
// background session, opens it via Provider.Open, and sends the detach key
// (Ctrl+]) as early as possible -- racing the client's own terminal setup,
// which is exactly the window in which a real, reproduced bug used to
// swallow the detach (see startClaudeAttachRaw's doc comment and
// TestStartClaudeAttachRawDeliversByteSentDuringChildStartupWindow for the
// mechanism). It asserts Open returns promptly (the client actually
// exited) and that the background session is still alive afterward, i.e.
// detaching never cancels or kills it.
func TestRealClaudeCtrlBracketDetachSurvivesEarlyRace(t *testing.T) {
	...
	p := &Provider{Path: claudePath}
	openErr := make(chan error, 1)
	go func() { openErr <- p.Open(ctx, session.Session{Key: session.Key{ID: id}}, slave, &output) }()
	...
	case err := <-openErr:
		if err != nil {
			t.Fatalf("Open did not detach cleanly: %v", err)
		}
	case <-time.After(15 * time.Second):
		t.Fatal("Open did not return after an early Ctrl+] -- the race reproduced")
	}
...
// `claude agents --json --all`, the same source of truth Provider.List
// uses.
