...
A new five-way outcome classifier, `classifyCursorExperimentOutcome` (`NO_REQUEST` / `FRONTEND_401` / `CAPTURE_FAILURE` / `CHAIN_INCOMPLETE` / `COMPLETE` — README "Failure categories"), makes the distinction the third run's ambiguity called for structural rather than left to reading prose: `NO_REQUEST` (the wheel input triggered no cursor-endpoint traffic at all — a trigger failure, not an authorization failure) is checked before `FRONTEND_401` (the real frontend issued at least one cursor-present request and every one so far came back 401 with zero successful pages), which is checked before the existing `CAPTURE_FAILURE`/`CHAIN_INCOMPLETE`/`COMPLETE` distinctions. All of the above is build/test-verified (`go build`, `go vet`, `go test -race`, `node --check`); **not yet re-run live.**

**Phase C — live full enumeration:** **NOT YET RUN to completion.** Three live runs so far have each surfaced a real bug or ambiguity in this harness rather than producing a trustworthy count either way — the self-fetch 401, the stale-capture freshness bug, and the self-fetch/real-frontend 401 ambiguity. A fourth live run, with the known-negative probe removed from the default path, real response-status counting, and real wheel input in place, is needed. `go run . -project-name agentsctl -cursor-experiment` is ready for the repository owner to re-run; it no longer prints a self-fetch 401 by default, instead printing `known self-fetch probe: skipped`, `navigate Project: PASS`, a `target region: ...` line, `real wheel (attempt N, tick M): ...` lines, `cursor response status: ...`, `failure category: ...`, then one `page=N conversation_count=M cursor_in=<fingerprint or "0"> cursor_out=<fingerprint or "<terminal>">` line per harvested page and the completeness/count/ordering/inclusion report described below.

```text
initial cursor: 0
pages fetched: NOT YET RUN with the real-wheel mechanism
terminal cursor observed: NOT YET RUN
cursor cycle: NOT YET RUN
failure category: NOT YET RUN
```
...
**Result: CONDITIONAL — implementation and pure-logic tests complete; three live runs found and fixed real bugs/ambiguities, mechanism confirmed reachable by direct manual observation, full enumeration still unproven.** The cursor-pagination mechanism (passive-capture-based, now driven by real Electron wheel input rather than a synthetic DOM scroll event or a self-issued fetch), dedupe, cycle-detection, freshness-watermarking, response-status classification, outcome classification, and local-ordering logic are all implemented and unit-tested against synthetic data matching the real captured schema. Live evidence has positively confirmed, from the operator's own manual interaction, that scrolling the Project's own view does load further cursor pages of this exact endpoint — the mechanism is reachable by SOME input path; what the third run left unresolved is whether the harness's own automated input reproduces that path, and the fourth pass addresses the ambiguity that run exposed (self-fetch vs. real-frontend 401) rather than resolving it live. What remains unproven is whether this account's Project cursor-chain actually terminates, how many conversations it contains, whether the real Electron wheel input actually reproduces the operator's manual observation, and whether any further bugs surface once a run gets far enough to try. Per the task's explicit instruction, this correction is not asserted as decided ahead of that live evidence: **Project-inclusive pagination: CONDITIONAL** pending a fourth `go run . -project-name agentsctl -cursor-experiment` run, with the known-negative probe removed from the default path and real wheel input in place, from a real authenticated terminal.
