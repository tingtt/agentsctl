...
**Phase C — live full enumeration:** **NOT YET RUN to completion.** Four live runs so far have each surfaced a real bug or ambiguity rather than producing a trustworthy count either way — the self-fetch 401, the stale-capture freshness bug, the self-fetch/real-frontend 401 ambiguity, and now the non-idempotent-next-cursor-token dedupe bug. A fifth live run, with this fix in place, is needed. `go run . -project-name agentsctl -cursor-experiment` remains ready for the repository owner to re-run.

```text
initial cursor: 0
pages fetched: 1 confirmed (cursor_in=0, 10 items) before the fourth run's capture-layer bug stopped enumeration; a genuine second page likely loaded (project_conversation_links doubled 10->20) but was never examined
terminal cursor observed: NOT YET RUN
cursor cycle: NOT YET RUN (not reached)
failure category: CAPTURE_FAILURE (fourth run) — fixed, not yet re-verified
frontend cursor-request authorization: CONFIRMED WORKING (200, not 401) — FRONTEND_401 ruled out
```
...
**Result: CONDITIONAL — implementation and pure-logic tests complete; four live runs found and fixed real bugs/ambiguities, the input mechanism AND frontend authorization are now both confirmed working, full enumeration still unproven.** The cursor-pagination mechanism (passive-capture-based, driven by real Electron wheel input), dedupe (now tolerant of a non-idempotent next-cursor token), cycle-detection, freshness-watermarking, response-status classification, outcome classification, and local-ordering logic are all implemented and unit-tested against synthetic data matching the real captured schema. The fourth live run positively confirmed, with the harness's OWN automated input (not just the operator's manual interaction from the second run), that real wheel input causes the real frontend to load more conversations (`project_conversation_links` doubled mid-scroll) and that the real frontend's own cursor-present requests return HTTP 200, never 401 — `FRONTEND_401` is ruled out for this account/session. What remains unproven is whether this account's Project cursor-chain actually terminates and how many conversations it contains, now that the dedupe fix should let a run get far enough to find out. Per the task's explicit instruction, this correction is not asserted as decided ahead of that live evidence: **Project-inclusive pagination: CONDITIONAL** pending a fifth `go run . -project-name agentsctl -cursor-experiment` run, with the dedupe fix in place, from a real authenticated terminal.
