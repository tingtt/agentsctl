...
- `globalConversationsPages`: sanitized, per-observation wire diagnostics for every `/backend-api/conversations` response seen so far — query parameter keys/values (opaque values length-redacted), the `hide_snorlax` flag, response top-level keys, a scalar-only `meta` (e.g. `limit`/`offset`/`total`), and raw item count. Added for the Phase 5 pagination follow-up (2026-09-11); never exposes conversation content or IDs.
- `globalConversationsPageItems`: sanitized, Project-filtered conversation items for one already-observed page, addressed by its `globalConversationsPages` sequence number. Added for the same follow-up.
