...
**New diagnostic (not yet live-verified):** rather than guess further, `summarizeCursorInVariants`/`printCursorInVariants` were added: when `dedupeCapturesByCursorIn` fails closed, the harvest now prints, per conflicting `CursorIn`, a privacy-safe forensic summary — `observations` (how many captures shared this `CursorIn`), `distinct_id_sets` (how many genuinely different conversation sets appeared), `distinct_next_cursors`, and the min/max item count — never a raw ID or cursor value. A low `distinct_id_sets` relative to `observations` (e.g. 2 sets appearing dozens of times each) would point to flickering between a small number of stable states (e.g. two backend replicas or cache layers disagreeing) rather than continuous drift; a `distinct_id_sets` close to `observations` would point the other way. This is diagnostic-only — it does not change the fail-closed decision above — and needs a seventh live run to produce real values. Build/test-verified (`go build`, `go vet`, `go test -race`, `node --check`).

**Phase C — live full enumeration:** **NOT YET RUN to completion.** Six live runs so far have each surfaced a real bug, ambiguity, or open question rather than producing a trustworthy count either way — the self-fetch 401, the stale-capture freshness bug, the self-fetch/real-frontend 401 ambiguity, the non-idempotent-next-cursor-token dedupe bug, and now two runs (one "quiet") both showing first-page churn whose cause is not yet identified. `go run . -project-name agentsctl -cursor-experiment` remains ready for the repository owner to re-run; the new forensic summary should make the next run's output more diagnostic even if it fails the same way again.
...
**Result: CONDITIONAL — implementation and pure-logic tests complete; six live runs found and fixed real bugs/ambiguities and surfaced an unresolved first-page-churn question whose cause is now less clear than initially hypothesized; the input mechanism AND frontend authorization are both confirmed working across two independent runs; full enumeration still unproven.** The cursor-pagination mechanism (passive-capture-based, driven by real Electron wheel input), dedupe (tolerant of a non-idempotent next-cursor token, still fail-closed on a genuinely different conversation set), cycle-detection, freshness-watermarking, response-status classification, outcome classification, forensic variant summaries, and local-ordering logic are all implemented and unit-tested against synthetic data matching the real captured schema. Across the fourth, fifth, and sixth live runs, real wheel input was positively confirmed to cause the real frontend to load more conversations, and the real frontend's own cursor-present requests were positively confirmed to return HTTP 200, never 401, in all three runs — `FRONTEND_401` is ruled out for this account/session. The fifth run's initial hypothesis — that first-page churn was caused by genuine, concurrent Project activity — was weakened, not confirmed, by the sixth run's attempt at a quiet window, which reproduced the identical failure with an even larger burst of cursor-present requests (64 vs. 34). The true cause (external account activity, an intrinsic frontend polling/prefetch behavior, or something else) is not yet identified; a new forensic diagnostic (`summarizeCursorInVariants`) is in place to gather the evidence needed to distinguish these on the next run. Per the task's explicit instruction, no correction is asserted as decided ahead of further evidence: **Project-inclusive pagination: CONDITIONAL**, pending a seventh live run with the new forensic diagnostic in place.
